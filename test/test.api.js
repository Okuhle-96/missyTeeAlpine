...
describe("The Garment API", function () {
  ...
  it("should have a test method", async () => {
    ...
    assert.deepStrictEqual({ name: '' }, response.body);
  });

  it("should be able to find 30 garments", async () => {
    const response = await supertest(app).get("/api/garments").expect(200);
  ...
  it("you should be able to add 2 Male & 3 Female garments", async () => {
    ...
    const femaleResult = await supertest(app).get(
      `/api/garments?gender=Female`
    );
    ...
    const updatedMaleResult = await supertest(app).get(
      `/api/garments?gender=Male`
    );
    assert.equal(15, updatedMaleResult.body.data.length);

    const updatedFemaleResult = await supertest(app).get(
      `/api/garments?gender=Female`
    );
  ...
  it("you should be able to remove all the Unisex garments", async () => {
    await supertest(app).delete(`/api/garments?gender=Unisex`).expect(200);
